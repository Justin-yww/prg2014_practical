...
// Define new class: Person 
class Person(val firstNameS : String, val lastNameS : String)
extends Database:
  ...
  var date = ObjectProperty[LocalDate](LocalDate.of(2003, 5, 3))
  ...
  def delete(): Try[Int] =
    if (!(isExist)) then  // This is backwards!
      Try(DB autoCommit { implicit session =>
        sql"""
        DELETE FROM person
        WHERE firstName = ${firstName.value}
